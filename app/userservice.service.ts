...
export class UserserviceService {
 ...
 //fundId:any = window.localStorage.getItem('fundId');
  ...
  createMf(funddetails:MutualFund):Observable<any> {
    this.panNo = window.localStorage.getItem('panNo');
    return this.http.post(`${this.baseUrlUserss}/addMutualFunds`,funddetails, {observe:'response'});
  }
  ...
  addProduct(account:Account): Observable<any>{
    const headers = new Headers;
    headers.append('Access-Control-Allow-Origin', '*');
    this.panNo = window.localStorage.getItem('panNo');
    console.log(account);
    return this.http.post(`${this.baseUrlUsers}/addAccount`,account, {observe:'response'})
  } 
